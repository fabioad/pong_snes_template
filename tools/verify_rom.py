#!/usr/bin/env python3
"""Sanity-check a 32 KiB LoROM image before hardware tests."""
...
def main() -> None:
    if len(sys.argv) != 2:
        fail("usage: verify_rom.py build/pong.sfc")
    ...
    native_reset_unused = struct.unpack_from("<H", data, 0x7FEC)[0]
    ...
    if rom_size != 0x05:
        fail(f"ROM size byte ${rom_size:02X}, expected $05 (32 KiB)")
    if sram_size != 0x00:
    ...
    if native_reset_unused == 0x0000:
        # Native UNUSED at $7FEC is documented as unused; ignore.
        pass

    title = name.decode("ascii", errors="replace").rstrip("\x00 ")
    print(f"verify: OK  {path}")
    print(f"  title     {title!r}")
    print(f"  map       LoROM SlowROM NTSC, ROM only, 32 KiB")
